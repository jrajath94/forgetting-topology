...
Figure~\ref{fig:topology} shows the per-component \fvs{} for Qwen3-8B and Llama-3.1-8B. Three structural patterns emerge consistently across seeds:
\begin{enumerate}
...
\begin{figure}[t]
  \centering
  \pfig{Per-component \fvs{} heatmap (layer $\times$ projection-kind) for Qwen3-8B (left) and Llama-3.1-8B (right). Warm = high vulnerability, cool = resistance.}{scripts/make\_fig\_topology.py + experiments/results/fvs\_qwen3\_8b.json + fvs\_llama31\_8b.json}
  \caption{Per-component \fvs{} for Qwen3-8B and Llama-3.1-8B. Warm colours indicate high vulnerability; cool colours indicate resistance.}
  \label{fig:topology}
\end{figure}
...
Spearman's $\rho$ between $\fvs^{\mathrm{Qwen}}$ and $\fvs^{\mathrm{Llama}}$ over architecturally-shared projection positions is \pend{spearman_qwen_llama} (\pend{spearman_qwen_llama_p}). This is within the $\rho > 0.6$ threshold our \S\ref{sec:method:transfer} pre-registered as the bar for \fvs{} to be amortised within a model family.
